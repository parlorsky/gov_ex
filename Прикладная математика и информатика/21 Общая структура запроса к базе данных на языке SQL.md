...
- **Общая структура запроса к базе данных на языке SQL:** запрос `SELECT` строится из блоков `SELECT`, `FROM`, `WHERE`, `GROUP BY`, `HAVING`, `ORDER BY`, `LIMIT/OFFSET`; общий шаблон дан в [[#Синтаксис SELECT|синтаксисе SELECT]].
- **Основные блоки оператора SELECT:** [[#Назначение блоков|блоки]] отвечают за выбор выражений, источник данных, фильтрацию строк, группировку, фильтрацию групп, сортировку и ограничение результата.
- **Последовательность выполнения блоков:** логически SQL выполняет запрос не в порядке записи: сначала [[#Логический порядок выполнения|FROM/JOIN]], затем фильтрация, группировка, агрегаты, `SELECT`, сортировка и ограничение результата.
