...
- **Общая структура запроса к базе данных на языке SQL:** запрос `SELECT` строится из блоков `SELECT`, `FROM`, `WHERE`, `GROUP BY`, `HAVING`, `ORDER BY`, `LIMIT/OFFSET`; общий шаблон дан в <a href="#синтаксис-select">синтаксисе SELECT</a>.
- **Основные блоки оператора SELECT:** <a href="#назначение-блоков">блоки</a> отвечают за выбор выражений, источник данных, фильтрацию строк, группировку, фильтрацию групп, сортировку и ограничение результата.
- **Последовательность выполнения блоков:** логически SQL выполняет запрос не в порядке записи: сначала <a href="#логический-порядок-выполнения">FROM/JOIN</a>, затем фильтрация, группировка, агрегаты, `SELECT`, сортировка и ограничение результата.
...
<a id="логический-порядок-выполнения"></a>
