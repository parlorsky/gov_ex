...
## TL;DR
- **Формат XML:** [[#XML|XML]] — строгий текстовый формат вложенных тегов с элементами, атрибутами, одним корневым элементом и декларацией кодировки.
- **Модель DOM:** [[#DOM|DOM]] представляет XML/HTML как дерево объектов в памяти, где можно ходить по родителям, детям и соседям.
...
- **Работа с ними с помощью библиотеки BeautifulSoup:** [[#BeautifulSoup|BeautifulSoup]] парсит HTML/XML, ищет узлы через `find`, `find_all`, `select` и позволяет читать или менять текст и атрибуты.
