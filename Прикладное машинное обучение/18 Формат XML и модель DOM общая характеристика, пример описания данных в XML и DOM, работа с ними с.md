...
## TL;DR
- **Формат XML:** <a href="#xml">XML</a> — строгий текстовый формат вложенных тегов с элементами, атрибутами, одним корневым элементом и декларацией кодировки.
- **Модель DOM:** <a href="#dom">DOM</a> представляет XML/HTML как дерево объектов в памяти, где можно ходить по родителям, детям и соседям.
...
- **Работа с ними с помощью библиотеки BeautifulSoup:** <a href="#beautifulsoup">BeautifulSoup</a> парсит HTML/XML, ищет узлы через `find`, `find_all`, `select` и позволяет читать или менять текст и атрибуты.
